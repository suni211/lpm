...
export const getMinecraftUUID = async (username: string): Promise<string | null> => {
  try {
...
// UUID로 마인크래프트 닉네임 조회
export const getMinecraftUsername = async (uuid: string): Promise<string | null> => {
  try {
    const response = await axios.get(
...
// 마인크래프트 스킨 URL 조회
export const getMinecraftSkin = (uuid: string): string => {
