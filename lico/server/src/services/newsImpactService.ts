...
class NewsImpactService {
  ...
  async startNewsImpact(newsId: string): Promise<void> {
    ...
    // 영향받는 주식 목록 조회
    const affectedStocks = await this.getAffectedStocks(news);
   ...
   */
  private async getAffectedStocks(news: any): Promise<any[]> {
    let sql = 'SELECT id, symbol, current_price, industry_id FROM stocks WHERE status = "ACTIVE"';
    const params: any[] = [];
   ...
   * AI 자동 매매 실행
   */
  private async executeAITrading(news: any, stocks: any[]): Promise<void> {
   ...
   * AI 주문 생성
   */
  private async createAIOrder(news: any, stock: any): Promise<void> {
    // AI 전용 지갑 조회 (없으면 생성)
    const aiWallet = await this.getOrCreateAIWallet();
    ...
    console.log(`[NewsImpact] AI 주문 생성: ${stock.symbol} ${orderType} ${quantity}주 @ ${price.toFixed(2)} G`);
  }

  /**
   * AI 전용 지갑 조회 또는 생성
   */
  private async getOrCreateAIWallet(): Promise<any> {
