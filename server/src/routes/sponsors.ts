...
router.get('/available', isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    const teamResult = await query(
      'SELECT id, current_tier, reputation FROM teams WHERE user_id = ?',
      [userId]
    );
    const team = teamResult[0];

    // 현재 계약 중인 스폰서 확인
    const currentSponsorResult = await query(`
      SELECT sponsor_id
      FROM team_sponsors
...
router.post('/contract/:sponsorId', isAuthenticated, async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 스폰서 정보 조회
    const [sponsorResult]: any = await client.query(
    ...
    const [existingResult]: any = await client.query(
      'SELECT * FROM team_sponsors WHERE team_id = ? AND sponsor_id = ? AND contract_end_date > NOW()',
      [team.id, sponsorId]
    );
    ...
    // 같은 타입의 스폰서가 있는지 확인
    const [sameTypeResult]: any = await client.query(`
      SELECT ts.*
      FROM team_sponsors ts
      JOIN sponsors s ON ts.sponsor_id = s.id
      WHERE ts.team_id = ?
...
router.post('/cancel/:contractId', isAuthenticated, async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 계약 정보 조회
    const [contractResult]: any = await client.query(
      'SELECT * FROM team_sponsors WHERE id = ? AND team_id = ?',
      [contractId, teamId]
    );
    ...
    // 계약 해지 (즉시 종료)
    await client.query(
      'UPDATE team_sponsors SET contract_end_date = NOW() WHERE id = ?',
      [contractId]
    );
...
router.post('/monthly-payment', isAuthenticated, async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 활성 계약 모두 조회
    const [contractsResult]: any = await client.query(`
    ...
    for (const contract of contractsResult) {
      ...
      // 마지막 지급일 업데이트
      await client.query(
        'UPDATE team_sponsors SET last_payment_date = NOW() WHERE id = ?',
        [contract.id]
      );
...
export async function payBonusIfConditionMet(
  ...
  try {
    ...
    for (const contract of contractsResult) {
      ...
      // 보너스 수령 기록
      await client.query(
        'UPDATE team_sponsors SET bonus_received = bonus_received + ? WHERE id = ?',
