 ...
 * 랭크 경기 시뮬레이션
 */
async function simulateMatch(team1Id, team2Id) {
    const client = await (0, db_1.getConnection)();
    try {
        await client.beginTransaction();
        // 팀 정보 가져오기
        const [team1Result] = await client.query('SELECT * FROM teams WHERE id = ?', [team1Id]);
        const [team2Result] = await client.query('SELECT * FROM teams WHERE id = ?', [team2Id]);
        ...
        // LP 변화 계산
        const lpChange = calculateLPChange(team1.current_tier, team2.current_tier, winnerId === team1Id);
        // 팀 전적 업데이트
        if (winnerId === team1Id) {
 ...
 * 팀 선수 정보 가져오기
 */
async function getTeamPlayers(client, teamId) {
    const [rosterResult] = await client.query('SELECT * FROM rosters WHERE team_id = ?', [teamId]);
    ...
    for (const pos of positions) {
        ...
        const [playerResult] = await client.query(`SELECT upc.*, pc.position, pc.mental, pc.team_fight, pc.cs_ability,
 ...
 * 페이즈 1: 라인전 (0~15분)
 */
function simulatePhase1(team1, team2) {
    let team1Score = 0;
    let team2Score = 0;
    const events = [];
    // 각 라인별 대결
 ...
 */
function simulatePhase2(team1, team2, phase1) {
 ...
 * 페이즈 3: 최종 한타 (30~45분)
 */
function simulatePhase3(team1, team2, phase2) {
    let team1Score = phase2.team1_score;
    let team2Score = phase2.team2_score;
    const events = [];
    // 최종 한타
    const team1FinalPower = team1.reduce((sum, p) => sum +
        p.mental +
        p.team_fight +
        p.judgment +
        p.power * CONDITION_BONUS[p.condition], 0);
    const team2FinalPower = team2.reduce((sum, p) => sum +
        p.mental +
        p.team_fight +
        p.judgment +
        p.power * CONDITION_BONUS[p.condition], 0);
 ...
 * LP 변화 계산
 */
function calculateLPChange(tier1, tier2, team1Won) {
    const tierValues = {
        BRONZE: 1,
        SILVER: 2,
        GOLD: 3,
 ...
 * 팀 통계 업데이트
 */
async function updateTeamStats(client, teamId, won, lpChange) {
    if (won) {
        await client.query(`UPDATE teams
       SET wins = wins + 1,
           win_streak = win_streak + 1,
           lp = lp + $1
       WHERE id = $2`, [lpChange, teamId]);
    }
    else {
        await client.query(`UPDATE teams
       SET losses = losses + 1,
 ...
 * 티어 승급/강등 체크
 */
async function checkTierPromotion(client, teamId) {
    const [teamResult] = await client.query('SELECT * FROM teams WHERE id = ?', [teamId]);
    ...
    if (newTier !== team.current_tier) {
        await client.query('UPDATE teams SET current_tier = ? WHERE id = ?', [newTier, teamId]);
    }
}
