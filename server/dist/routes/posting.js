...
// 경매 목록 조회
router.get('/auctions', auth_1.isAuthenticated, async (req, res) => {
    try {
...
// 내 경매 목록 조회
router.get('/my-auctions', auth_1.isAuthenticated, async (req, res) => {
    try {
...
router.post('/create', auth_1.isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        const teamId = teamResult[0].id;
        // 카드 소유 확인
        const [cardCheck] = await client.query('SELECT * FROM user_player_cards WHERE id = ? AND team_id = ? AND is_in_roster = false', [userCardId, teamId]);
        ...
        const [postingInsert] = await client.query(`
    ...
    `, [userCardId, teamId, startingPrice, buyoutPrice, endTime]);
        const newPosting = await (0, db_1.query)('SELECT * FROM postings WHERE id = ?', [postingInsert.insertId]);
        // 카드를 경매 상태로 변경
        await client.query('UPDATE user_player_cards SET is_on_auction = true WHERE id = ?', [userCardId]);
        await client.commit();
        res.json({
            message: '경매가 등록되었습니다',
...
// 입찰
router.post('/bid', auth_1.isAuthenticated, async (req, res) => {
    const client = await (0, db_1.getConnection)();
    try {
        const userId = req.user?.id;
        ...
        await client.beginTransaction();
        // 팀 정보 조회
        const [teamResult] = await client.query('SELECT id, balance FROM teams WHERE user_id = ?', [userId]);
    ...
    `, [auctionId, team.id, bidAmount]);
      ...
      `, [team.id, auction.user_card_id]);
            // 판매자에게 금액 지급
            await client.query('UPDATE teams SET balance = balance + ? WHERE id = ?', [bidAmount, auction.seller_team_id]);
            // 구매자 잔액 차감
            await client.query('UPDATE teams SET balance = balance - ? WHERE id = ?', [bidAmount, team.id]);
            // 경매 종료
            await client.query("UPDATE postings SET status = 'completed', winner_team_id = ?, final_price = ? WHERE id = ?", [team.id, bidAmount, auctionId]);
            await client.commit();
            return res.json({
...
// 경매 취소
router.post('/cancel/:auctionId', auth_1.isAuthenticated, async (req, res) => {
    const client = await (0, db_1.getConnection)();
    try {
        const userId = req.user?.id;
        ...
        const teamId = teamResult[0].id;
        // 경매 정보 조회
        const [auctionResult] = await client.query('SELECT * FROM postings WHERE id = ? AND seller_team_id = ? AND status = ?', [auctionId, teamId, 'active']);
        if (auctionResult.length === 0) {
            await client.rollback();
        ...
        const auction = auctionResult[0];
        // 입찰이 있는지 확인
        const [bidCheck] = await client.query('SELECT COUNT(*) as count FROM auction_bids WHERE auction_id = ?', [auctionId]);
        if (bidCheck[0].count > 0) {
            await client.rollback();
            return res.status(400).json({ error: '입찰이 있는 경매는 취소할 수 없습니다' });
        }
        // 경매 취소
        await client.query("UPDATE postings SET status = 'cancelled' WHERE id = ?", [auctionId]);
        // 카드 상태 복구
        await client.query('UPDATE user_player_cards SET is_on_auction = false WHERE id = ?', [auction.user_card_id]);
        await client.commit();
...
// 경매 상세 정보 조회
router.get('/:auctionId', auth_1.isAuthenticated, async (req, res) => {
    try {
        const { auctionId } = req.params;
        // 경매 정보
    ...
    `, [auctionId]);
        ...
        // 입찰 내역
        const bidsResult = await (0, db_1.query)(`
      SELECT
        ab.*,
