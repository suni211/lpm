...
router.get('/', auth_1.isAuthenticated, async (req, res) => {
    try {
        ...
        // 로스터 정보
        const rosterResult = await (0, db_1.query)('SELECT * FROM rosters WHERE team_id = ?', [teamId]);
        ...
        for (const pos of positions) {
            ...
            if (playerId) {
                const playerResult = await (0, db_1.query)(`SELECT upc.*, pc.card_name, pc.card_image, pc.position, pc.cost,
...
// 로스터에 선수 배치
router.post('/assign', auth_1.isAuthenticated, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
        ...
        const teamId = teamResult[0].id;
        // 선수 카드 조회
        const cardResult = await (0, db_1.query)(`SELECT upc.*, pc.position, pc.cost
       FROM user_player_cards upc
       JOIN player_cards pc ON upc.player_card_id = pc.id
       WHERE upc.id = ? AND upc.user_id = ?`, [userCardId, req.user.id]);
        ...
        for (const pos of ['top', 'jungle', 'mid', 'adc', 'support']) {
            if (pos === position) {
                ...
                if (currentPlayerId) {
                    const currentPlayerResult = await (0, db_1.query)(`SELECT pc.cost FROM user_player_cards upc
             JOIN player_cards pc ON upc.player_card_id = pc.id
             WHERE upc.id = ?`, [currentPlayerId]);
        ...
        if (oldPlayerId) {
            await (0, db_1.query)('UPDATE user_player_cards SET in_roster = false WHERE id = ?', [oldPlayerId]);
        }
        // 새 선수 배치
        await (0, db_1.query)(`UPDATE rosters SET ${position}_player_id = ?, total_cost = ?, updated_at = CURRENT_TIMESTAMP
       WHERE team_id = ?`, [userCardId, newTotalCost, teamId]);
        // 선수를 로스터에 추가
        await (0, db_1.query)('UPDATE user_player_cards SET in_roster = true WHERE id = ?', [userCardId]);
...
// 로스터에서 선수 제거
router.post('/remove', auth_1.isAuthenticated, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
        ...
        // 선수의 코스트 가져오기
        const playerResult = await (0, db_1.query)(`SELECT pc.cost FROM user_player_cards upc
       JOIN player_cards pc ON upc.player_card_id = pc.id
       WHERE upc.id = ?`, [playerId]);
        const cost = playerResult[0].cost;
        const newTotalCost = roster.total_cost - cost;
        // 로스터에서 제거
        await (0, db_1.query)(`UPDATE rosters SET ${position}_player_id = NULL, total_cost = ?, updated_at = CURRENT_TIMESTAMP
       WHERE team_id = ?`, [newTotalCost, teamId]);
        // 선수 상태 업데이트
        await (0, db_1.query)('UPDATE user_player_cards SET in_roster = false WHERE id = ?', [playerId]);
...
// 사용 가능한 선수 목록 (포지션별)
router.get('/available/:position', auth_1.isAuthenticated, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
        ...
        // 해당 포지션의 사용 가능한 선수들
        const playersResult = await (0, db_1.query)(`SELECT upc.*, pc.card_name, pc.card_image, pc.position, pc.cost,
              pc.mental, pc.team_fight, pc.cs_ability, pc.vision,
              pc.judgment, pc.laning, pc.power, pc.rarity
       FROM user_player_cards upc
       JOIN player_cards pc ON upc.player_card_id = pc.id
       WHERE upc.user_id = ? AND pc.position = ? AND upc.in_roster = false
       ORDER BY pc.power DESC`, [req.user.id, positionMap[position]]);
